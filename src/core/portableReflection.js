 ...
 *
 * @param {object} effectRes Loaded raw Tr2EffectRes graph.
...
export function buildEffectBodyReflection(effectRes, permutationIndex)
{
    ...
    {
        throw new TypeError(
            "Portable effect reflection requires a loaded Tr2EffectRes with GetShaderByIndex"
        );
    }
...
function emitPass(pass, techniqueKey, passIndex)
{
    ...
    const stages = [];
    for (let stageType = 0;
        stageType < Tr2RenderContextEnum.SHADER_TYPE_COUNT;
        stageType += 1)
    {
...
function validatePass(pass, counts, source)
{
    ...
    for (const stage of pass.stages)
    {
        ...
        if (stage.stageType >= Tr2RenderContextEnum.SHADER_TYPE_COUNT
        ...
        if (stage.stageName !== tr2ShaderStageName(stage.stageType))
        {
